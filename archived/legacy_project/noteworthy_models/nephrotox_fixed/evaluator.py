...
class Evaluator:
    ...
    def cross_validate(
        ...
        for fold, (tr, val) in enumerate(skf.split(X_combined, y), 1):
            ...
            _log_fold(fold, n_folds, met, logger)
...
def _log_fold(fold: int, n_folds: int, met: dict, logger_) -> None:
    """Print a clean single-line metric summary for one CV fold."""
    logger_.info(
        f"    ┌─ Fold {fold}/{n_folds} results ──────────────────────────────────────────┐"
    )
    logger_.info(
        f"    │  AUC      : {met['auc']:.4f}    ACC        : {met['acc']:.4f}         │"
    )
    logger_.info(
        f"    │  Recall   : {met['se']:.4f}    Specificity: {met['specificity']:.4f}         │"
    )
    logger_.info(
        f"    │  F1       : {met['f1']:.4f}    Kappa      : {met['kappa']:.4f}         │"
    )
    logger_.info(
        f"    │  MCC      : {met['mcc']:.4f}                                     │"
    )
...
def _log_summary(agg: dict, label: str, logger_) -> None:
    """Print a full metric summary table after CV completes."""
    W = 66
    logger_.info("")
    logger_.info("═" * W)
    logger_.info(f"  {label} — Summary (mean ± std across folds)")
    logger_.info("═" * W)
    logger_.info(f"  {'Metric':<18} {'Mean':>10} {'± Std':>10}  {'Range'}")
    logger_.info("  " + "─" * (W - 2))
    labels = {
        "auc":         "AUC (ROC)",
        "acc":         "Accuracy",
        "se":          "Recall / Sensitivity",
    ...
    for k, display in labels.items():
        if k not in agg:
            continue
        v = agg[k]
        mean = v["mean"]
        std  = v.get("std", 0.0)
        logger_.info(f"  {display:<22} {mean:>8.4f}   ±{std:>7.4f}")
    logger_.info("═" * W)
    logger_.info("")


def _compare(agg: dict, paper: dict, label: str, logger_) -> None:
    """Head-to-head table: our model vs paper benchmark."""
    W = 66
    logger_.info(f"  Comparison vs {label}")
    logger_.info("  " + "─" * (W - 2))
    ...
    for k, display in labels.items():
        ...
        if delta > 0.01:
            ...
            status = "✗ behind"
        logger_.info(
            f"  {display:<18} {pv:>8.3f}  {ov:>8.3f}  {sign}{delta:>7.3f}  {status}"
        )
    logger_.info("  " + "─" * (W - 2))
    logger_.info("")
